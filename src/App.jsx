...
export default function App() {
    ...
    const handleShare = async () => {
        try {
            ...
            text += `*Total Bill: ${formatCurrency(grandTotal)}*\n`;
            text += `(Subtotal: ${formatCurrency(totalSubtotal)})\n\n`;

            text += `*Who Owes What:*\n`;

            users.forEach(user => {
                const total = userTotals[user.id]?.total || 0;
                if (total > 0) {
                    text += `👤 ${user.name}: *${formatCurrency(total)}*\n`;
                }
            });

            text += `\nGenerated by Smart Split Bill 🤖`;
            ...
            if (navigator.share) {
                ...
                alert('Bill summary copied to clipboard! You can paste it in WhatsApp/Telegram.');
            }
        } catch (err) {
    ...
    const saveBill = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Backend server returned an error.');
            }

            alert(`Bill "${billTitle}" saved successfully to database!`);
        } catch (err) {
            console.error(err);
            if (err.name === 'TypeError' && err.message.includes('Failed to fetch')) {
                alert('Failed to save: Could not connect to the backend server.\n\nPlease ensure the backend (node index.js) is running on port 3000.');
            } else {
            ...
            <div className="p-6 pb-32 animate-fade-in">
                ...
                <div className="fixed bottom-0 left-0 right-0 p-6 bg-white border-t border-slate-100 max-w-md mx-auto flex flex-col gap-3">
                    ...
                    <div className="flex gap-3">
                        ...
                        <Button variant="primary" onClick={handleShare} className="flex-1" icon={Share2}>
                            Share
                        </Button>
                    </div>
                </div>
